...
interface MemoryDao {
    ...
    // ------------------------------------------------------------------
    // Embedding-aware queries
    // ------------------------------------------------------------------

    /**
     * Returns all active memories that have a non-null embedding BLOB,
     * ordered by importance (DESC) so the caller can apply cosine-similarity
     * re-ranking on the full candidate set without a secondary DB round-trip.
     *
     * Room cannot perform vector arithmetic in SQL, so the float comparison
     * is done in Kotlin inside [com.agente.autonomo.memory.MemorySearchEngine].
     */
    @Query("""
    ...
    """)
    suspend fun getAllEmbeddedMemories(): List<Memory>

    /**
     * Returns all active memories whose embedding is still NULL —
     * i.e. rows that pre-date the embedding engine or whose inference failed.
     * Used by the background back-fill job.
     */
    @Query("""
        SELECT * FROM memories
        WHERE is_archived = 0
        ...
        LIMIT :limit
    """)
    suspend fun getMemoriesWithoutEmbedding(limit: Int = 100): List<Memory>

    /**
     * Persist the computed embedding BLOB for an existing memory row.
     * Only [Memory.embedding] and [Memory.updatedAt] are modified.
     */
    @Query("UPDATE memories SET embedding = :embeddingBlob, updated_at = :now WHERE id = :memoryId")
    suspend fun updateEmbedding(memoryId: String, embeddingBlob: ByteArray, now: Date = Date())
